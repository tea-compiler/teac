...
impl<'a> ParseContext<'a> {
    ...
    ///
    /// Returns [`Error::Grammar`](super::common::Error::Grammar) if no
    /// recognisable inner rule is found.
    ...
    /// Parses an `assignment_stmt` node (`left_val = right_val;`) into a boxed
    /// [`ast::AssignmentStmt`].
    ///
    /// Both operands are required;
    /// [`Error::Grammar`](super::common::Error::Grammar) is returned if either
    /// is absent.
    ...
    /// effects: `func(args);`.  Returns
    /// [`Error::Grammar`](super::common::Error::Grammar) if the expected
    ...
    /// The grammar has no dedicated `else` node: body statements accumulate in
    /// `if_stmts` until the `kw_else` token flips accumulation into
    /// `else_stmts`.
    ///
    /// Returns [`Error::Grammar`](super::common::Error::Grammar) if no
    /// condition is found.
